...
bool EncoderLibJpeg::encode(const StreamConfiguration &cfg, libcamera::FrameBuffer *buffer, Image *image, std::string outFileName)
{
    ...
    */
    ...
    jpeg_set_quality(&compress_, 92, TRUE);

    jpeg_start_compress(&compress_, TRUE);

    unsigned char *src = image_.bits();
